...
function createProgram(gl, nextVertexShaderSource, nextFragmentShaderSource) {
  const vertexShader = createShader(gl, gl.VERTEX_SHADER, nextVertexShaderSource)
  const fragmentShader = createShader(gl, gl.FRAGMENT_SHADER, nextFragmentShaderSource)
...
export function DitheringShader({
  ...
  useEffect(() => {
    ...
    const renderWidth = Math.max(1, Math.floor(size.width * devicePixelRatio))
    const renderHeight = Math.max(1, Math.floor(size.height * devicePixelRatio))
    ...
    const render = () => {
      ...
      if (locations.u_resolution) gl.uniform2f(locations.u_resolution, renderWidth, renderHeight)
      ...
      if (locations.u_shape) gl.uniform1f(locations.u_shape, DitheringShapes[shape] ?? DitheringShapes.simplex)
      if (locations.u_type) gl.uniform1f(locations.u_type, DitheringTypes[type] ?? DitheringTypes["8x8"])
  ...
  }, [colorBack, colorFront, pxSize, shape, size.height, size.width, speed, type])
